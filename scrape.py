...
def scrape_race_data(start_year,start_mon, end_year, end_mon, race_csv_name, pay_csv_name, endpoint):
    ...
    for i in range(1,endpoint): # 1から372までのページをクロール
        ...
        for td in tds:
            ...
            # print(result_table)
            # CSVファイルに書き込み
            with open(race_csv_name, mode="a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                # データ本体
            ...
            with open(pay_csv_name, mode="a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                # データ本体
